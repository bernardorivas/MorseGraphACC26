...
# MOSEK ApS, 2018-2025
#
import mosek, sys, re, signal, os, platform
import numpy as np 
...
helptext = [
    ...
    ('fixsol solname', 'Fix all variables to a specific solution'),
    ('fixintsol', 'Fix all integer variables to a specific solution'),
    ('infsub', 'Replace current problem with its infeasible subproblem'),
    ('dualize', 'Replace the current problem by its dual'),
    ...
    ('ptf', 'Print the PTF representation of the problem.'),
    ('optserver [url]', 'Use an OptServer to optimize'),
    ('ls', 'List the current folder'),
    ('failonerr', 'Fail on any wrong command'),
    ('exit', 'Leave')
]
...
helpdetail = {
    ...
    'fixsol': 'fixsol solname\n\n Fix all variables to the values in solution solname (int, bas, itg)',
    'fixintsol': 'fixintsol\n\n Fix all integer variables to the values from the integer solution',
    'infsub': 'infsub\n\nReplace the current problem by its infeasible subproblem (likely much smaller). Available for linear and conic problems.',
    'dualize': 'dualize\n\nReplace the current problem by its dual. Available for linear and conic problems.',
    ...
    'failonerr': 'failonerr\n\nFail on any wrong command instead of just printing a message.'
}
...
def msk_help_detail(cmd):
    if cmd in helpdetail: 
        msk_print(helpdetail[cmd])
    else: 
        msk_print("Unknown command")
        msk_fail()
...
# Make all or integer variables fixed to values from a particular solution
def msk_fixsol(stype, onlyint = False):
    soltypes = {'itr' : mosek.soltype.itr,
                'bas' : mosek.soltype.bas,
                'itg' : mosek.soltype.itg }
    sol = soltypes[stype]
    if task.solutiondef(sol):
        numvar = task.getnumvar()
        xx = task.getxx(sol)
        idx = list(range(numvar))
        if onlyint:
            vartypes = task.getvartypelist(idx)
            idx = [ j for j in range(numvar) if vartypes[j] == mosek.variabletype.type_int ]
            xx = [ xx[j] for j in idx ]
        numfixvar = len(idx)
        task.putvarboundlist(idx, [mosek.boundkey.fx] * numfixvar, xx, xx)
    else:
        msk_print("Solution not available")
        msk_fail()
...
def msk_infsub():
    try:
        ...
        msk_print("Operation not possible")
        msk_fail()

# Replace with the dual problem
def msk_dualize():
    try:
        global task
        newtask = None
        newtask = task.getdualproblem()
        if newtask:
            task.__del__()
            task = newtask
            task.set_Stream(mosek.streamtype.log, msk_write_log)
...
def msk_ls():
    ...
    if plat in ['Windows']:
        ...
        msk_print(f'Unknown platform {plat}')
        msk_fail()
...
def msk_anapro_struct_cones():
    ...
    isSDP = task.getnumbarvar() > 0 or any(task.getdomaintype(task.getaccdomain(accidx)) in [mosek.domaintype.svec_psd_cone] for accidx in range(task.getnumacc()))
    if isSDP:
        msk_anapro_schur()
        msk_print('')
        ...
        try:
            if cmd == 'exit': 
                ...
                if len(l) > 0: 
                    ...
                    msk_print('Missing basename')
                    msk_fail()            
            elif cmd == 'writejsonsol':
                if len(l) > 0: 
                    msk_writejsonsol(l[0])
                else: 
                    msk_print('Missing name')
                    msk_fail()
            elif cmd == 'ptf':
                msk_printdata(mosek.dataformat.ptf)
            elif cmd == 'param':
                if len(l) == 2: 
                    msk_putparam(l[0], l[1])
                elif len(l) == 1: 
                    msk_getparam(l[0])
                else: 
                    msk_getparam('')
                ...
                if len(l) > 0 and l[0] in ['itr', 'bas', 'itg']: 
                    msk_fixsol(l[0], False)
                else: 
                    msk_print('Missing or wrong solution')
                    msk_fail()
            elif cmd == 'fixintsol':
                msk_fixsol('itg', True)
            elif cmd == 'hist':
                ...
                if len(l) > 0: 
                    msk_truncate(float(l[0]))
                else: 
                    msk_print('Missing epsilon')
                    msk_fail()
            elif cmd == 'resobj':
                if len(l) > 0: 
                    msk_resobj(float(l[0]))
                else: 
                    msk_resobj(0.0)
            elif cmd == 'optserver':
                if len(l) > 0: 
