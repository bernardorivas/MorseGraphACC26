...
namespace mosek
{
  namespace fusion
  {
    namespace Utils
    {
      ...
      bool IntMap::hasItem (int64_t key) { return _impl->hasItem(key); }
      int  IntMap::getItem (int64_t key) { return _impl->getItem(key); }
      void IntMap::setItem (int64_t key, int val) { _impl->setItem(key,val); }
      std::shared_ptr<monty::ndarray<int64_t,1>> IntMap::keys() { return _impl->keys(); }
