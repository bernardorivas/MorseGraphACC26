...
# ==============================================================================
# hat_f from Learned Data (Corrected)
# ==============================================================================

# Switching surface polynomial - learned approximation of x[0]**2 - 1
# Adding small perturbations to simulate learning errors
hat_polynomials = [
    lambda x: x[0]**2 - 1.002 + 0.001*x[0] - 0.0005*x[1],  # Learned switching surface with small errors
]

# Vector fields - learned approximation of Van der Pol dynamics
# Including small perturbations to simulate learning from data
hat_vector_fields = [
    # Mode 0: |x0| < 1 (cubic force) - learned approximation
    lambda x: np.array([x[1],
                        mu*(0.998 - 1.001*x[0]**2)*x[1] - 0.999*x[0]**3 + 0.002*x[0]]),
    # Mode 1: |x0| > 1 (linear force) - learned approximation
    lambda x: np.array([x[1],
                        mu*(1.002 - 0.998*x[0]**2)*x[1] - 1.001*x[0] - 0.001*x[0]**2]),
]
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Piecewise VdP Morse graph analysis with caching')
    ...
    if 1 not in force_methods and cache_exists(output_dir, 1):
        ...
        combined_morse_graph, combined_roas = post_processing_example_2(
    ...
    combined_basin_box_counts = {i: len(combined_roas[node]) for i, node in enumerate(combined_morse_graph.nodes())}
    ...
    if 2 not in force_methods and cache_exists(output_dir, 2):
        ...
        # Post-processing
        print("\nPost-processing morse graph...")
        combined_morse_graph2, combined_roas2 = post_processing_example_2(
    ...
    # Apply colors to morse_graph2 for consistent visualization
    ...
    combined_basin_box_counts = {i: len(combined_roas2[node]) for i, node in enumerate(combined_morse_graph2.nodes())}
    ...
    if 3 not in force_methods and cache_exists(output_dir, 3):
        ...
        # Post-processing
        print("\nPost-processing morse graph...")
        combined_morse_graph3, combined_roas3 = post_processing_example_2(
    ...
    # Apply colors to morse_graph3 for consistent visualization
    ...
    combined_basin_box_counts = {i: len(combined_roas3[node]) for i, node in enumerate(combined_morse_graph3.nodes())}
    ...
    if 4 not in force_methods and cache_exists(output_dir, 4):
        ...
        # Post-processing
        print("\nPost-processing morse graph...")
        combined_morse_graph4, combined_roas4 = post_processing_example_2(
    ...
    # Apply colors to morse_graph4 for consistent visualization
    ...
    combined_basin_box_counts = {i: len(combined_roas4[node]) for i, node in enumerate(combined_morse_graph4.nodes())}
    ...
    if 5 not in force_methods and cache_exists(output_dir, 5):
        ...
        # Post-processing
        print("\nPost-processing morse graph...")
        combined_morse_graph5, combined_roas5 = post_processing_example_2(
    ...
    # Apply colors to morse_graph5 for consistent visualization
    ...
    combined_basin_box_counts = {i: len(combined_roas5[node]) for i, node in enumerate(combined_morse_graph5.nodes())}
    ...
    combined_roas_list = [res['combined_roas'] for res in results_summary]
